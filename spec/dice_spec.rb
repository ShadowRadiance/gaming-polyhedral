...
module Polyhedral
  RSpec.describe Dice do
    ...
    context 'with five specific dice rolls (3,2,1,4,1)' do
      def die_with_face(face)
        Die.new(6).tap { |dbl| allow(dbl).to receive(:face).and_return(face) }
      end

      let(:d_1) { die_with_face(1) }
      let(:d_2) { die_with_face(2) }
      let(:d_3) { die_with_face(3) }
      let(:d_4) { die_with_face(4) }

      subject do
        Dice.new([d_3, d_2, d_1, d_4, d_1])
      end

      describe '#reroll_ones' do
        it 'rerolls ones and only ones' do
          subject.roll
          expect(d_1).to receive(:roll).twice
          subject.reroll_ones
        end
      end
      ...
      describe '#drop_lowest' do
        it 'drops a die roll' do
          ...
          end.to change { subject.values.size }.by(-1)
        end
